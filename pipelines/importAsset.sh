...
debug=${@: -1}
    ...
    if [ -z "skip_connector_flows" ]; then
      echo "Missing template parameter skip_connector_flows"
      exit 1
    fi
    if [ -z "skip_connectors" ]; then
      echo "Missing template parameter skip_connectors"
      exit 1
    fi    
...
if [ ${synchProject} == true ]; then
  ...
  # skip_connector=("_db" "_sap" "_connector" "_wrapper")
  ...
  if [ ${#fs_files[@]} -gt 0 ]; then
    ...
    for filename in ./assets/flowservices/*.zip; do 
        ...
        ##04/02/2025 end , attempt to skip connector deployment if skip_connector_flows is set to true

        if [ ${skip_connector_flows} == true ]; then
          skip=false
          # for pattern in "${skip_connectors[@]}"; do
          for pattern in $skip_connectors; do
            if [[ "${base_name}" == *"${pattern}" ]]; then
              echo "Skipping ${base_name} as it matches the pattern ${pattern}."
              skip=true;  
              break
            fi
          done
        ...
        # if [[ $base_name == *db* ]]; then
        #   echo "Skipping file containing 'db' : $filename"
        #   continue
        # fi
        echod $base_name${filename%.*}
        echod $parent_name
